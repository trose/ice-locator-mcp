...
/** @internal */
exports.nodeJSProcessExitedAfterLoggingUserActionableMessage = 32;
...
function printErrorUsageAndExit(message) {
...
function main(args) {
    ...
    if (argv["parallel"]) {
        if (isNaN(parseInt(argv["parallel"], 10))) {
            return printErrorUsageAndExit(`error: --parallel flag must specify a number: ${argv["parallel"]} is not a number`);
        }
    }
    ...
    v8Hooks.isInitializedAsync().then(() => {
        ...
        /*isErrorReported:  */ (err) => loggedErrors.has(err));
