#!/usr/bin/env python3
"""
MCP Client to search for Hispanic/Latino names using the ICE Locator MCP Server.
...
class MCPNameSearcher:
    """MCP client for searching names in the ICE database."""
    
    def __init__(self, output_csv: str = "mcp_search_results.csv"):
    ...
    def save_result(self, full_name: str, result: Dict):
        ...
        if result and result.get("status") == "found" and result.get("results"):
            ...
            row = [
                full_name,
                detainee.get("first_name", ""),
                detainee.get("last_name", ""),
                detainee.get("alien_number", ""),
                detainee.get("date_of_birth", ""),
                detainee.get("country_of_birth", ""),
        ...
        try:
            ...
            response = await client_session.call_tool(
                ...
                arguments={
                    ...
                    "last_name": last_name,
                    "date_of_birth": "1980-01-01",  # Generic date if not specified
                    "country_of_birth": "Mexico"    # Most common Hispanic country
                }
            )
...
async def main():
    ...
    searcher = MCPNameSearcher("mcp_search_results.csv")
    
    # Input file path
    input_file = "organized/hispanic_full_names_extended_medium.txt"
    ...
    # Run the search - limit to first 20 names for testing
    await searcher.run_search(full_path, max_names=20)
