#!/usr/bin/env python3
"""
HTTP client to search Hispanic/Latino names using ICE Locator MCP Server over HTTP.
"""
...
class HispanicNameSearcherHTTP:
    """Class to search Hispanic names using ICE Locator MCP Server over HTTP."""
    
    def __init__(self, server_url: str = "http://127.0.0.1:8081/mcp", output_csv: str = "hispanic_names_search_results.csv"):
    ...
    async def search_name(self, full_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            if result and "result" in result:
                ...
                if isinstance(result_data, dict) and result_data.get("status") == "found" and result_data.get("results"):
                    self.found_count += 1
                ...
                print(f"No result returned for {full_name}")
    ...
    async def search_names_batch(self, names: List[str], batch_size: int = 3, delay: float = 2.0):
        ...
        if tools:
            ...
            print("Failed to connect to server or list tools")
            return
        ...
        for i in range(0, len(names), batch_size):
            ...
            for name in batch:
                result = await self.search_name(name)
                if result:
...
async def main():
    ...
    print("Starting search of Hispanic/Latino names using ICE Locator MCP over HTTP...")
    ...
    searcher = HispanicNameSearcherHTTP("http://127.0.0.1:8081/mcp", "hispanic_names_search_results.csv")
